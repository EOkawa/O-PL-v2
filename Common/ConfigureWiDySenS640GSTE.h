#ifndef CONFIGUREWIDYSENS640GSTE_H_INCLUDED
#define CONFIGUREWIDYSENS640GSTE_H_INCLUDED

/** This sample configuration is specific to WiDySenS 640-GSTE cameras                **/
void ConfigureDevice( NITLibrary::NITDevice* dev )
{
    ...
    /* Each time a parameter is changed, the ConfigObserver is called          */
		...
		cout << "PixelFormat : " << dev->paramStrValueOf( "PixelFormat" ) << endl;                //Note the presence "Str"
		//Query as double
        ...
        //
		//Parameters are sent to the camera immediatly, calling updateConfig permit to retrieve the updated parameter in the configObserver
		dev->setParamValueOf( "OutputType", "GREY" );        //When OutputType GREY is selected an AGC is applied in the camera
		unsigned int sensor_width = dev->sensorWidth();
		unsigned int sensor_height = dev->sensorHeight();
		dev->setParamValueOf( "OffsetX", 0u );               //Note the strict typing of the value
		dev->setParamValueOf( "Width", sensor_width );       //Note the strict typing of the value
		//dev->setParamValueOf( "NumberOfColumns", 640 );    //This line cannot be compiled error would be ambiguous type for value
		dev->setParamValueOf( "OffsetY", 0u );               //Set Y position of begin of frame
		dev->setParamValueOf( "Height", sensor_height );     //Set frame height
        dev->setParamValueOf( "ExposureTime", 5000.0 );      //Set Exposure Time
		dev->updateConfig();                                 //ConfigObserver is updated
...
#endif // CONFIGUREWIDYSENS640GSTE_H_INCLUDED
